(function( $ ){
    ...
    $.fn.pan = function(options) {
        ...
        var onInterval = function() {
			...
			content.css('transform', 'translate3d(' + (Math.floor(offset.x*100)/100) + 'px, ' + (Math.floor(offset.y*100)/100) + 'px, 0px)');
			//console.log('test');
        }
		...
		var setCenter = function() {
			mousePosition.x = containerSize.width / 2;
			mousePosition.y = containerSize.height / 2;
		}

		this.destroy = function() {
			console.log('destroy image pan :: ' + interval);
			clearInterval(interval);
			interval = null;
			$(document).off('touchmove MSPointerMove pointermove');
			$(document).off('mouseleave');
			this.off('touchstart MSPointerDown pointerdown');
			this.off('touchend MSPointerUp pointerup');
		}

        $(document).on('touchmove MSPointerMove pointermove', function(evt) {
            mousePosition.x = evt.pageX - container.offset().left;
            mousePosition.y = evt.pageY - container.offset().top;
            mouseOver = true;

			//플래시라이트 효과
			/* // evt.touches[0] 에서 undefined 오류 발생
        ...
        this.on('touchstart MSPointerDown pointerdown', function(evt) {
            dragging = true;
            return false; //Prevents FF from thumbnailing & dragging
        });

        this.on('touchend MSPointerUp pointerup', function(evt) {
            dragging = false;
            lastMousePosition = null;
        });
        ...
        var interval = setInterval(onInterval, updateInterval);
